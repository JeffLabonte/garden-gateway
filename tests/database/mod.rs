...
fn given_get_all_configurations_when_has_five_configurations_then_should_return_five_configs() {
    get_database_connection().test_transaction::<_, Error, _>(|connection| {
        ...
        let sensor_a_configuration = create_configuration("Sensor A".to_string(), 61, connection);
        let sensor_b_configuration = create_configuration("Sensor B".to_string(), 62, connection);
        let sensor_c_configuration = create_configuration("Sensor C".to_string(), 63, connection);
        let sensor_d_configuration = create_configuration("Sensor D".to_string(), 64, connection);
        let sensor_e_configuration = create_configuration("Sensor E".to_string(), 65, connection);
...
fn given_get_confgurations_from_schedule_id_when_two_configuration_linked_config_then_should_return_two(
    ...
    get_database_connection().test_transaction::<_, Error, _>(|connection| {
        let sensor_a_configuration = create_configuration("Sensor A".to_string(), 60, connection);
        let sensor_b_configuration = create_configuration("Sensor B".to_string(), 61, connection);
...
fn given_get_configuration_dependencies_from_config_id() {
    get_database_connection().test_transaction::<_, Error, _>(|connection| {
        let water_detector_config = configurations::table
            .filter(configurations::dsl::sensor_name.eq("water_detector".to_string()))
            .first::<Configuration>(connection)
            .expect("Error loading water detector config");

        let configuration_deps =
            get_configuration_dependencies_from_config_id(water_detector_config.id, connection);
